...
<body>
	...
	<form method="post" onsubmit="return formCheck(this);">
		<h3 id="title" onclick="changeHome()">DRONG</h3>
		<div id="msg"></div>
		<input type="text" name="id" value="${cookie.cid.value }" placeholder="아이디입력" autofocus>
		<input type="password" name="pwd" placeholder="비밀번호">
        <button>로그인</button>
        <div>
            <label><input type="checkbox" name="rememberId" ${not empty cookie.cid.value ? "checked" : ""}> 아이디 기억</label> 
            <a href='<c:url value="regist" />'>회원가입</a>
        </div>
        <input type="hidden" name="url" value="${param.url}">
	</form>
	<script>
		if(${param.loginError != null}) document.querySelector("#msg").innerText="아이디와 비밀번호를 확인하세요."
